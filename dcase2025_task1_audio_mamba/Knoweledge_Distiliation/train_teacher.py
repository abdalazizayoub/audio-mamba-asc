...
# --- EXPLICITLY USING YOUR DATASET SCRIPT ---
...
class EfficientATTeacherModule(pl.LightningModule):
    def __init__(self, config):
        ...
        # This matches the LabelEncoder from your dcase25.py perfectly!
    ...
    def forward(self, x, mixup_lam=None, mixup_index=None):
        # Safely catches the [1, 32000] waveform from your TimeShiftDataset
        if x.dim() == 4:
        ...
        # 2. BULLETPROOF LOG SCALING (No NaNs!)
        x = torch.log(x + 1e-8)
        
        # 3. Add Channel Dimension for the CNN
        x = x.unsqueeze(1)
    ...
    def training_step(self, train_batch, batch_idx):
        # Your dataset returns: waveform, file, label, device, city
        ...
        if torch.rand(1).item() < mixup_prob:
            ...
            y_hat = self.forward(x, mixup_lam=lam, mixup_index=index)
...
def train(config):
    ...
    # Automatically converts your roll_sec (0.1) into the 3200 integer your script expects!
    ...
    # We use your test_set for the validation loop
    val_dl = DataLoader(
        get_test_set(), 
    ...
    if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
        macs, params_bytes = complexity.get_torch_macs_memory(pl_module.net, input_size=(1, 1, 128, 101))
        wandb_logger.experiment.config.update({"MACs": macs, "Params_Bytes": params_bytes}, allow_val_change=True)
    ...
    # EXPLICITLY passing your train and val dataloaders
...
if __name__ == '__main__':
    ...
    parser.add_argument("--experiment_name", type=str, default="EfficientAT_mn10_as_Mixup")
